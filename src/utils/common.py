...
def _log_shap_importance(final_pipeline, X_train, top_n: int = 10) -> dict:
    try:
        ...
        raw_shap = np.asarray(shap_values.values)
        if raw_shap.ndim == 3:
            raw_shap = raw_shap[:, :, 1]
        ...
        local_shap_path = Path("models") / "shap_importance.json"
        local_shap_path.parent.mkdir(parents=True, exist_ok=True)
        with open(local_shap_path, "w", encoding="utf-8") as f:
            json.dump(top_features, f, indent=2)
        logger.info("SHAP importance persisted locally at %s", local_shap_path)
